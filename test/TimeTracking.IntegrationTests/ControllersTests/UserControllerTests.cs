...
{
    public class UserControllerTests:Request<Startup>
    {
        [SetUp]
        ...
        public async Task GetUserById_WhenFound_ReturnsTeam()
        {
            ...
            var httpResponse = await _client.GetAsync(UserControllerRoutes.BaseRoute+"/"+userId);
        ...
        public async Task AddUserToTeam_WhenTeamNotFound_ReturnTeamNotFound()
        {
            ...
            var httpResponse = await Post(UserControllerRoutes.AddUserToTeam,request);
            httpResponse.EnsureSuccessStatusCode();
            var response = await httpResponse.BodyAs<ApiResponse<TimeTrackingUserDetailsDto>>();
        ...
        public async Task AddUserToTeam_WhenRequestNotValid_ReturnValidationErrors()
        {
            ...
            httpResponse = await Post(UserControllerRoutes.AddUserToTeam,request);
            httpResponse.EnsureSuccessStatusCode();
        ...
        [TestCase(1,2,1,2)]
        [TestCase(1,3,1,3)]
        public async Task GetAllUsers_WhenRequestValid_ReturnsValidPageResult(int page,int size,int expectedPage,int expectedSize)
        {
            ...
            var httpResponse = await _client.GetAsync(UserControllerRoutes.AllUsers+"?"+pagedRequest.GetQueryString());
            httpResponse.EnsureSuccessStatusCode();
